...
### Ranks 
For each `i = 1, …, n`, the *local rank* <code>r<sub>i</sub></code> defined to be the value such that <code>a<sub>i</sub></code> is the <code>r<sub>i</sub></code>th smallest value of the set <code>{a<sub>1</sub>,a<sub>2</sub>,..., a<sub>i</sub>}</code>.  
...
The local rank sequence <code>r<sub>1</sub>, r<sub>2</sub>, ..., r<sub>n</sub></code> uniquly determines the global rank sequence and vice versa. Given the gloabl rank seqence, for each `i = 1, …, n`, <code>r<sub>i</sub></code> is the value such that <code>g<sub>i</sub></code> is the <code>r<sub>i</sub></code>th smallest value of the set <code>{g<sub>1</sub>,g<sub>2</sub>,..., g<sub>i</sub>}</code>. Given the local ranks, one can arrange <code>a<sub>1</sub>,a<sub>2</sub>,..., a<sub>n</sub></code> in increasing order by putting 
The gobal rank <code>g<sub>i</sub></code> is determined by the index of <code>a<sub>i</sub></code> in the ordered sequence. 
